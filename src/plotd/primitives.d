...
Point convertCoordinates(const Point point, const Bounds orig_bounds, const Bounds new_bounds)
{
    double new_x = new_bounds.min_x + (new_bounds.max_x - new_bounds.min_x) * (point
        .x - orig_bounds.min_x) / (orig_bounds.max_x - orig_bounds.min_x);
    double new_y = new_bounds.min_y + (new_bounds.max_y - new_bounds.min_y) * (point
        .y - orig_bounds.min_y) / (orig_bounds.max_y - orig_bounds.min_y);
    return Point(new_x, new_y);
}

unittest
{
    assert(convertCoordinates(Point(0.5, 0.1), Bounds(-1, 1, -1, 1), Bounds(50, 100, -100, -50)) == Point(87.5, -72.5));
}
...
class Lines
{
    ...
    private  : LineState[LineId] lines;
    LineId last_id = 0;
