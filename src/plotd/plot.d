...
CONTEXT drawFunction(CONTEXT)(double delegate(double) func, Bounds bounds,
    CONTEXT context)
{
    auto points = iota(bounds.min_x, bounds.max_x, bounds.width / 100.0).map!(a => Point(a,
        func(a)));
    auto from = points[0];
    foreach (to; points[1 .. $])
    {
...
PlotState!("png") createPlotStatePNG(Bounds plotBounds, Bounds marginBounds)
{
    ...
    plot.surface = createPlotSurface(plot.marginBounds.max_x.to!int, plot
        .marginBounds.max_y.to!int);
    // setup axes
    plot.axesContext = axesContextFromSurface(plot.surface, plot.plotBounds, plot
        .marginBounds);
    plot.axesContext = drawAxes(plot.plotBounds, plot.axesContext);
    plot.plotContext = plotContextFromSurface(plot.surface, plot.plotBounds, plot
        .marginBounds);
    return plot;
}
    ...
    Bounds marginBounds)
{
    ...
    {
        plot.surface = createPlotSurfacePDF(plot.name, plot.marginBounds.max_x
            .to!int, plot.marginBounds.max_y.to!int);
    }
    else if (T == "svg" && CAIRO_HAS_SVG_SURFACE)
    {
        plot.surface = createPlotSurfaceSVG(plot.name, plot.marginBounds.max_x
            .to!int, plot.marginBounds.max_y.to!int);
    }
    else
    {
...
unittest
{
    ...
    {
        auto plot = createPlotState!"pdf"("test", Bounds(0, 1, 0, 1), Bounds(10, 100, 10, 100));
        assert(plot.name == "test.pdf");
        assert(plot.surface.getType() == cairo.SurfaceType
...
unittest
{
    auto plot = createPlotState!"png"("test", Bounds(0, 1, 0, 1), Bounds(10, 100, 10, 100));
    assert(plot.name == "test.png");
    assert(plot.surface.getType() == cairo.SurfaceType.CAIRO_SURFACE_TYPE_IMAGE);
}
...
void drawRange(RANGE, alias string T)(RANGE range, PlotState!T plot)
{
    if (!range.empty)
    {
        ...
        while (!range.empty)
        {
            ...
            range.popFront;
            plot.plotContext = drawLine(firstPoint, nextPoint, plot.plotContext);
...
unittest
{
    static if (CAIRO_HAS_PDF_SURFACE)
    {
        auto plot = createPlotState!"pdf"("testLine", Bounds(0, 1, 0, 1), Bounds(10, 100, 10, 100));
...
/// Draw function on our plot
void drawFunction(alias string T)(double delegate(double) func, PlotState!T plot)
{
    iota(plot.plotBounds.min_x, plot.plotBounds.max_x, plot.plotBounds.width / 100.0)
...
unittest
{
    static if (CAIRO_HAS_PDF_SURFACE)
    {
        auto plot = createPlotState!"pdf"("testPoint", Bounds(0, 1, 0, 1), Bounds(10, 100, 10, 100));
...
{
    plot.axesContext = plotd.drawing.drawXLabel(xlabel, plot.plotBounds, plot
        .axesContext);
}
...
{
    plot.axesContext = plotd.drawing.drawYLabel(ylabel, plot.plotBounds, plot
        .axesContext);
}
