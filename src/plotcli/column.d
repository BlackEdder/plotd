...
struct Formats
{
    ...
    this(size_t noColumns)
    {
        ...
        else _formats = parseDataFormat("h,..")._formats;
    }
    ...
    @property Format front()
    {
        ...
        if (_formats.length == 1 && fm.mode == "..")
        {
            ...
            if (prevFormats.length == 2 && prevFormats[0].plotID.length > 0
                && prevFormats[1].plotID.length > 0)
            {
                pfm.plotID = pfm.plotID[0 .. $ - 1] ~ (prevFormats[1].plotID.back.to!char
    ...
    void popFront()
    {
        if (_formats.length <= 3 && prevFormats.length == 0 && _formats.back.mode
            == "..")
        {
...
bool validFormat(Formats formats, string[] columns)
{
    ...
    if (formats.empty)
        return false;
    if (formats._formats.length == columns.length || formats._formats.back
        .mode == "..")
    {
